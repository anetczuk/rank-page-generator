...
if [ -f "$PAGE_PATH" ]; then
	...
	chromium --headless --window-size=1920,1080 "file://${PAGE_PATH}?horn=yes&wings=no" --screenshot="$OUT_IMG_PATH"
	mogrify -trim "$OUT_IMG_PATH"
	convert -bordercolor \#EBEDEF -border 20 "$OUT_IMG_PATH" "$OUT_IMG_PATH"
	convert "$OUT_IMG_PATH" -strip "$OUT_IMG_PATH"
	exiftool -overwrite_original -all= "$OUT_IMG_PATH"
...
PAGE_PATH="$OUT_DIR/subpage/category_0.html"
if [ -f "$PAGE_PATH" ]; then
...
PAGE_PATH="$OUT_DIR/subpage/match_0.html"
if [ -f "$PAGE_PATH" ]; then
